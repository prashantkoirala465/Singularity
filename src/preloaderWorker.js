...
class Preloader {
  ...
  drawTriangle() {
    ...
    const firstMinX = Math.abs(
      -sideLength / 2 +
        Math.abs(
          (-height / 2 + firstSmallHeight / 2 + height / 2) / Math.sqrt(3),
        ),
    );

    // Draw small triangles
    for (
      let y = -height / 2 + smallHeight / 2;
      y <= height / 2 - smallHeight / 2;
      y += smallHeight
    ) {
      const minX = -sideLength / 2 + Math.abs((y + height / 2) / Math.sqrt(3));
      const maxX =
        sideLength / 2 - Math.abs((y + height / 2) / Math.sqrt(3)) + 1;
      ...
      for (
        ...
        if (x + smallSideLength / 2 <= maxX - smallSideLength) {
          this.drawSmallTriangle(
            x + offsetX + smallSideLength / 2,
            y + offsetY,
            smallSideLength,
            "white",
            true,
            offsetWhite,
          ); // Reverse small triangles
          this.drawSmallTriangle(
            x + offsetX + smallSideLength / 2,
            y + offsetY,
            smallSideLength,
  ...
  drawSmallTriangle(x, y, length, fillStyle, flipped, offset = 0) {
    ...
    // PlayButton Scale
    scale =
      MathHelper.mix(scale, offset * 0.9, this.playButtonState) +
      Math.sin(this.elapsed) * 0.3 * this.playButtonState;
  ...
  initStars() {
    ...
    for (let i = 0; i < this.initStarsPopulation; i++) {
      this.stars[i] = new Star(
        i,
        Math.floor(Math.random() * this.cw),
        Math.floor(Math.random() * this.ch),
      );
      //stars[i].draw();
    }
  ...
  showPreloader() {
    ...
    let interval = (this.animationInterval = setInterval(() => {
      ...
      this.animationState =
        1.0 - this.easeOutCubic(((Date.now() - startTime) / 1000) * 1.6);
    }));
  }
...
onmessage = (e) => {
  ...
  const handlers = {
    init: () => {
      canvas = data.canvas;
      window = data.window;
      window.preloader = canvas;

      preloaderObject = new Preloader();
    },

    hidePreloader: () => {
      preloaderObject.hidePreloader();
    },

    showPreloader: () => {
      preloaderObject.showPreloader();
    },
  };
